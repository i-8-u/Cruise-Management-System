...
import { ArrowLeft, User, Mail, Phone, Home, Package, Calendar, MessageSquare, UserCheck, UserRoundPen, EyeOff, Eye, KeySquare} from "lucide-react"
  ...
  useEffect(() => {
    const fetchVoyager = async () => {
      if (isEditing) {
        setLoading(true)
        try {
          const docRef = doc(db, "voyagers", id)
          ...
          console.error("Error fetching voyagers:", error)
        } finally {
          setLoading(false)
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        ...
        const crewDocRef = doc(db, "voyagers", id);
        await updateDoc(crewDocRef, updatedData);
        alert("Voyager updated successfully!");
        navigate("/admin/voyagers")
      } else {
        // Create user in Firebase Authentication
        const { email, password, ...newVoyagerData } = formData;
        ...
        await createUserWithEmailAndPassword(auth, email, password)
          .then(async (userCredential) => {
            ...
            await addDoc(collection(db, "voyagers"), newVoyager);
            alert("Voyager added successfully!");
      ...
      console.error("Error saving voyager:", error);
